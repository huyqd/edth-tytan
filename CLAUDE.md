...
- **`evaluate.py`**: Evaluation pipeline for stabilization quality assessment
  ...
  - **Cached original metrics**: Use `--model original` to pre-compute original video metrics once
    - Saves to `output/original_metrics.json` by default
    ...
    - **Only loads split frames**: When using `--split-set test`, only loads test set frames (not all frames)
...
- **Automatic metrics display**: Shows evaluation metrics below each model if available
  - Stabilization models show improvement metrics and quality scores
  - "Raw" shows baseline original video metrics (from `evaluate.py --model original`)
...
- `output/original_metrics.json` - Original video metrics (shows when "Raw" is selected, auto-generated by `evaluate.py --model original`)
