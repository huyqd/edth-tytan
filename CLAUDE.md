...
- **`evaluate.py`**: Evaluation pipeline for stabilization quality assessment
  ...
  - Outputs per-flight and aggregate statistics
...
```bash
# 1. Run stabilization on all data (disable split with empty string)
python src/inference.py --split ""

# 2. Evaluate results (disable split info)
python src/evaluate.py --model baseline --split "" --save-json baseline_all.json
```
...
# 2. Run stabilization on test set only (default split file: data/data_split.json)
python src/inference.py --split-set test

# 3. Evaluate test set results (default: loads data/data_split.json)
python src/evaluate.py --model baseline --save-json baseline_test.json
